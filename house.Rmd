...
model_xgb <- xgboost(data = as.matrix(train_treat), 
                     label = train_treat$SalePrice,  # column of outcomes
                   nrounds = 70,       # number of trees to build
...
```{r}
xgb_test = select(test_treat, -SalePrice)
xgb_test$SalePrice = predict(model_xgb, as.matrix(xgb_test))
```
